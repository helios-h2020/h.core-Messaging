...
class StreamMessageValidator {
    ...
    async validateGroupKeyRequest(streamMessage) {
        ...
        if (!StreamID_1.StreamIDUtils.isKeyExchangeStream(streamMessage.getStreamId())) {
            throw new StreamMessageError_1.default(`Group key requests can only occur on stream ids of form ${`${StreamID_1.StreamIDUtils.KEY_EXCHANGE_STREAM_PREFIX}{address}`}.`, streamMessage);
        }
        const groupKeyRequest = GroupKeyRequest_1.default.fromStreamMessage(streamMessage);
        const sender = streamMessage.getPublisherId();
        const recipient = StreamID_1.StreamIDUtils.getRecipient(streamMessage.getStreamId());
        await StreamMessageValidator.assertSignatureIsValid(streamMessage, this.verify);
    ...
    async validateGroupKeyResponseOrAnnounce(streamMessage) {
        ...
        if (!StreamID_1.StreamIDUtils.isKeyExchangeStream(streamMessage.getStreamId())) {
            throw new StreamMessageError_1.default(`${streamMessage.messageType} can only occur on stream ids of form ${`${StreamID_1.StreamIDUtils.KEY_EXCHANGE_STREAM_PREFIX}{address}`}.`, streamMessage);
        }
        await StreamMessageValidator.assertSignatureIsValid(streamMessage, this.verify);
