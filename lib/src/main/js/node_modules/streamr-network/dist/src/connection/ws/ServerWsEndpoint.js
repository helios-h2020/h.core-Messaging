...
class ServerWsEndpoint extends AbstractWsEndpoint_1.AbstractWsEndpoint {
    ...
    acceptConnection(ws, duplexStream, peerId, remoteAddress) {
        ...
        duplexStream.on('error', (error) => {
            this.logger.error('Duplex stream error: ' + error.stack);
        });
        ws.on('pong', () => {
            connection.onPong();
        });
...
function cleanSocket(httpServer, config) {
    httpServer.on('error', (err) => {
        ...
        ServerWsConnection_1.staticLogger.info('socket in use, trying to recover: %s', config);
        ServerWsConnection_1.staticLogger.trace('checking if socket in use by another server');
        ...
        clientSocket.on('error', (err) => {
            ...
            try {
                ServerWsConnection_1.staticLogger.trace('cleaning unused socket: %s', config);
        ...
        clientSocket.once('connect', () => {
            ...
            ServerWsConnection_1.staticLogger.error('Another server already running on socket: %s', config);
            process.exit(1);
        });
...
async function startHttpServer(config, privateKeyFileName = undefined, certFileName = undefined) {
    ...
    try {
        ...
        ServerWsConnection_1.staticLogger.info(`listening on %s`, JSON.stringify(config));
    }
    catch (err) {
        // Kill process if started on host/port, else wait for Unix Socket to be cleaned up
        if (typeof config !== "string") {
            ServerWsConnection_1.staticLogger.error(err);
            process.exit(1);
        }
        else {
            await (0, events_1.once)(httpServer, 'listening');
