...
class Metrics {
    ...
    addRecordedMetric(name, windowSizeInSeconds = 5) {
        ...
        const _speedometer = new Speedometer_1.Speedometer(windowSizeInSeconds);
    ...
    addFixedMetric(name, initialValue = 0) {
        this.verifyUniqueness(name);
    ...
    async report() {
        const queryResults = await Promise.all(Object.entries(this.queriedMetrics)
...
class MetricsContext {
    ...
    async report(clearLast = false) {
        const entries = await Promise.all(Object.entries(this.metrics)
