...
const log = (0, log_1.Debug)('DataUnion');
function getMessageHashes(tr) {
    ...
    const hashes = sigEventArgsArray.map((eventArgs) => (0, keccak256_1.keccak256)(eventArgs[1]));
    return hashes;
}
    ...
    return tx.wait().catch((err) => {
        ...
        if (err.body) {
            ...
            if (retries > 0 && msg.includes('ancient block sync')) {
                ...
                return (0, utils_1.sleep)(retryInterval).then(() => waitForTx(tx, { retries: retries - 1, retryInterval }));
            }
        }
        throw err;
    });
...
class DataUnion {
    ...
    constructor(contractAddress, sidechainAddress, client) {
        ...
        this.contractAddress = (0, address_1.getAddress)(contractAddress);
        this.sidechainAddress = (0, address_1.getAddress)(sidechainAddress);
        this.client = client;
    }
    getAddress() {
    ...
    async join(secret) {
        ...
        const response = await this.client.rest.post(['dataunions', this.contractAddress, 'joinRequests'], body);
        if (secret) {
            await (0, utils_1.until)(async () => this.isMember(memberAddress));
        }
        return response;
    }
    /**
    ...
    async isMember(memberAddress) {
        const address = (0, address_1.getAddress)(memberAddress);
    ...
    async _createWithdrawSignature(amountTokenWei, to, withdrawn, signer) {
        const message = to
            + (0, bytes_1.hexZeroPad)(bignumber_1.BigNumber.from(amountTokenWei).toHexString(), 32).slice(2)
            + this.getSidechainAddress().slice(2)
            + (0, bytes_1.hexZeroPad)(withdrawn.toHexString(), 32).slice(2);
        const signature = await signer.signMessage((0, bytes_1.arrayify)(message));
        return signature;
    }
    // Query functions
    /**
     ...
     */
    static async getVersion(contractAddress, client) {
        return new Contracts_1.default(client).getVersion(contractAddress);
    }
    async getVersion() {
    ...
    async createSecret(name = 'Untitled Data Union Secret') {
        const res = await this.client.rest.post(['dataunions', this.contractAddress, 'secrets'], {
            name,
        });
        return res === null || res === void 0 ? void 0 : res.secret;
    }
    /**
    ...
    async withdrawAllToMember(memberAddress, options) {
        const address = (0, address_1.getAddress)(memberAddress); // throws if bad address
    ...
    async getWithdrawAllToMemberTx(memberAddress, sendToMainnet = true) {
        const a = (0, address_1.getAddress)(memberAddress); // throws if bad address
        const duSidechain = await this.getContracts().getSidechainContract(this.contractAddress);
        const ethersOverrides = this.client.ethereum.getDataUnionOverrides();
    ...
    async withdrawAllToSigned(memberAddress, recipientAddress, signature, options) {
        const from = (0, address_1.getAddress)(memberAddress); // throws if bad address
        const to = (0, address_1.getAddress)(recipientAddress);
     ...
     * Version 2.0: admin fee was collected in DataUnionMainnet
     */
    async setAdminFee(newFeeFraction) {
    ...
    static async _createSetBinanceRecipientSignature(to, signer, ethereumUtils) {
        ...
        const message = to
            + (0, bytes_1.hexZeroPad)(nextNonce.toHexString(), 32).slice(2)
    ...
    async _executeWithdraw(getWithdrawTxFunc, recipientAddress, options = {}) {
        ...
        if (!payForTransport) {
            if (waitUntilTransportIsComplete) {
                ...
                await (0, utils_1.until)(async () => !(await getBalanceFunc()).eq(balanceBefore), retryTimeoutMs, pollingIntervalMs).catch((e) => {
        ...
        if (waitUntilTransportIsComplete) {
            ...
            await (0, utils_1.until)(async () => !(await getBalanceFunc()).eq(balanceBefore), retryTimeoutMs, pollingIntervalMs);
        }
        return ambTr;
    }
    ...
    async transportMessage(messageHash, pollingIntervalMs = 1000, retryTimeoutMs = 300000, ethersOptions = {}) {
        ...
        await (0, utils_1.until)(async () => helper.requiredSignaturesHaveBeenCollected(messageHash), retryTimeoutMs, pollingIntervalMs);
