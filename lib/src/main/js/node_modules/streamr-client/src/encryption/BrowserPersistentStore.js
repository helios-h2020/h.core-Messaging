...
class BrowserPersistentStore {
    constructor({ clientId, streamId }) {
        ...
        this.store = (0, idb_keyval_1.createStore)(this.dbName, 'GroupKeys');
    }
    async has(key) {
    ...
    async set(key, value) {
        ...
        await (0, idb_keyval_1.set)(key, value, this.store);
        return had;
    }
    async delete(key) {
        ...
        await (0, idb_keyval_1.del)(key, this.store);
        return true;
    }
    async clear() {
        const size = await this.size();
        await (0, idb_keyval_1.clear)(this.store);
        return !!size;
    }
    async size() {
        const allKeys = await (0, idb_keyval_1.keys)(this.store);
