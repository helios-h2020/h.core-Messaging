...
class HeliosMessagingStreamrLibp2p : HeliosMessaging {
    var directMessaging: HeliosDirectMessaging? = null
    private var node: StreamrInstance? = null
    ...
    companion object {
        const val TAG = "HeliosMessagingStreamrLibp2p"
        const val NODE_JS_PACKAGE = "streamr-node-files.zip"
        const val NODE_JS_ENTRY = "lib/streamr-mobile-client.js"
        const val SHARED_PREFERENCES_FILE = "helios-node-streamr-prefs"

        @JvmStatic
        val instance: HeliosMessagingStreamrLibp2p by lazy {
            HeliosMessagingStreamrLibp2p()
        }
    }
    ...
    fun start(identity: HeliosIdentityInfo, appContext: Context = context!!) {
        ...
        val nodeDir = StreamrInstance.prepareNodeAsset(appContext, NODE_JS_PACKAGE)
        val nodeInstance = StreamrInstance(arrayOf(File(nodeDir, NODE_JS_ENTRY).path))
        val dm = HeliosDirectMessagingStreamrLibp2p(nodeInstance)
        ...
        eventPump = thread(name = "helios-libp2p-event") {
            ...
            while (!Thread.currentThread().isInterrupted) {
                try {
                    ...
                    Log.d(TAG, "MESSAGE EVENT " + msg)
                    if (msg !is StreamrMessageEvent) {
                        continue
                    }
                    ...
                    if (msg.name == "pubsub:message") {
                        ...
                        val pubsubmsg = buildPubSubMessage(msg.stringData)
                        if (pubsubmsg == null) {
                            Log.d(TAG, "Error when parsing pubsub message")
                            continue
                        }
			            Log.d(TAG, "pubsub created")

                        if (pubsubmsg.topic.isNullOrEmpty()) {
                            Log.d(TAG, "Topic check failed")
                            continue
                        }
                        ...
                        Log.d(TAG, "Message created ${topic} ${message.getMessage()}" )
        ...
        if (sharedRelayAddresses == null) {
            relayAddresses =
                listOf("/ip4/130.188.225.47/tcp/28327/ws/p2p/QmUFSesHRPzJxJzLccTtYc4QNoiDSEe3r6KgBwLdTrxpKh")
        } else {
        ...
        val swarmKeyProtocol =
            sharedPreferences.getString("swarmKeyProtocol", "/key/swarm/psk/1.0.0/")
        val swarmKeyEncoding = sharedPreferences.getString("swarmKeyEncoding", "/base16/")
        val swarmKeyData = sharedPreferences.getString(
            "swarmKeyData",
            "bc22c182f9f47f3daebf961a0f5242e4731ae7bf566869b530b527d9ef2d3fb0"
        )
        val walletAddress = sharedPreferences.getString("walletAddress", "empty")
        val walletPrivateKey = sharedPreferences.getString("walletPrivateKey", "empty")
        ...
        val rv =
            nodeInstance.callMethod("start", arrayOf(networkIdentity, clientOptions)) as? Map<*, *>
        val receivedPrivateId = rv?.get("privateNetworkId") as? String?
        peerId = rv?.get("networkId") as? String
        val receivedWalletAddress = rv?.get("walletAddress") as? String?
        val receivedWalletPrivateKey = rv?.get("walletPrivateKey") as? String?
        ...
        if ((receivedWalletAddress != null) && (receivedWalletPrivateKey != null)) with(
            sharedPreferences.edit()
        ) {
            putString("walletAddress", receivedWalletAddress)
            putString("walletPrivateKey", receivedWalletPrivateKey)
            commit()
        }
        privatePeerId = receivedPrivateId

        Log.i(TAG, "Publish ego: /helios/ego/peer/${identity.userUUID}")
        Log.i(TAG, "Received address: ${receivedWalletAddress}")
        // TODO
        // thread {
            try {
    ...
    private fun buildPubSubMessage(str: String): P2pPubSubMessage? {
        try {
            val json = JSONObject(str)
            val topicList: List<String> = listOf(json.getString("topic"))
            val pubsubmsg = P2pPubSubMessage(
                json.getString("networkId"),
                json.getString("subscriptionId"),
                topicList,
                json.getString("data").toByteArray(StandardCharsets.UTF_8)
            )
            Log.d(TAG, "pubsub created")
            return pubsubmsg
        } catch (e: Exception) {
    ...
    fun announceTag(tag: String) {
        Log.d(TAG, "AnnounceTag disabled in SpringApp")
        /*
        lock.withLock {
    ...
    fun unannounceTag(tag: String) {
        Log.d(TAG, "unannounceTag disabled in SpringApp")
        /*
        lock.withLock {
    ...
    fun observeTag(tag: String) {
        Log.d(TAG, "observeTag disabled in SpringApp")
        /*
        ...
        subscribe(topic, object : HeliosMessageListener {
            override fun showMessage(topic: HeliosTopic?, message: HeliosMessage?) {
                Log.d(TAG, "observeTag showMessage")
                try {
                    val htag: HeliosEgoTag =
    ...
    fun unobserveTag(tag: String) {
        Log.d(TAG, "unobserveTag disabled in SpringApp")
        /*
        val topic = HeliosTopic("/helios/tag/$tag", "")
        unsubscribe(topic)
         */
