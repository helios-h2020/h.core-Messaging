...
/**
 * Parse private key test string and compare key components to stored values.
 */
class RsaPrivateKeyReaderTest {
    ...
    @Test
    fun testModulus() {
        val keyReader = RsaPrivateKeyReader(privateKey)
        if (keyReader.modulus == null) {
            Assert.assertEquals(1,2)
        }
        val buffer: ByteArray = keyReader.modulus!!.toByteArray()
        Assert.assertEquals(buffer.size, modulus.size)
        for (i in buffer.indices) {
            Assert.assertEquals(buffer[i], modulus[i])
        }
    }

    @Test
    fun testPublicExponent() {
        val keyReader = RsaPrivateKeyReader(privateKey)
        if (keyReader.publicExponent == null) {
            Assert.assertEquals(1,2)
        }
        Assert.assertEquals(keyReader.publicExponent!!.toInt(), publicExponent)
    }

    @Test
    fun testPrivateExponent() {
        val keyReader = RsaPrivateKeyReader(privateKey)
        if (keyReader.privateExponent == null) {
            Assert.assertEquals(1,2)
        }
        val buffer: ByteArray = keyReader.privateExponent!!.toByteArray()
        Assert.assertEquals(buffer.size, privateExponent.size)
        for (i in buffer.indices) {
            Assert.assertEquals(buffer[i], privateExponent[i])
        }
    }
    ...
    fun testPrime1() {
        val keyReader = RsaPrivateKeyReader(privateKey)
        if (keyReader.prime1 == null) {
            Assert.assertEquals(1,2)
        }
        val buffer: ByteArray = keyReader.prime1!!.toByteArray()
        Assert.assertEquals(buffer.size, prime1.size)
        for (i in buffer.indices) {
            Assert.assertEquals(buffer[i], prime1[i])
        }
    }
    ...
    fun testPrime2() {
        val keyReader = RsaPrivateKeyReader(privateKey)
        if (keyReader.prime2 == null) {
            Assert.assertEquals(1,2)
        }
        val buffer: ByteArray = keyReader.prime2!!.toByteArray()
        Assert.assertEquals(buffer.size, prime2.size)
        for (i in buffer.indices) {
            Assert.assertEquals(buffer[i], prime2[i])
        }
    }
    ...
    fun testExponent1() {
        val keyReader = RsaPrivateKeyReader(privateKey)
        if (keyReader.exponent1 == null) {
            Assert.assertEquals(1,2)
        }
        val buffer: ByteArray = keyReader.exponent1!!.toByteArray()
        Assert.assertEquals(buffer.size, exponent1.size)
        for (i in buffer.indices) {
            Assert.assertEquals(buffer[i], exponent1[i])
        }
    }
    ...
    fun testExponent2() {
        val keyReader = RsaPrivateKeyReader(privateKey)
        if (keyReader.exponent2 == null) {
            Assert.assertEquals(1,2)
        }
        val buffer: ByteArray = keyReader.exponent2!!.toByteArray()
        Assert.assertEquals(buffer.size, exponent2.size)
        for (i in buffer.indices) {
            Assert.assertEquals(buffer[i], exponent2[i])
        }
    }
    ...
    fun testCoefficient() {
        val keyReader = RsaPrivateKeyReader(privateKey)
        if (keyReader.coefficient == null) {
            Assert.assertEquals(1,2)
        }
        val buffer: ByteArray = keyReader.coefficient!!.toByteArray()
        Assert.assertEquals(buffer.size, coefficient.size)
        for (i in buffer.indices) {
            Assert.assertEquals(buffer[i], coefficient[i])
        }
    }
